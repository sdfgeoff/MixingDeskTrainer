...
function EQTrainer() {
    ...
    return (
        <div style={{ background: COLORS.background, color: COLORS.text, display: 'flex', flexDirection: 'column', gap: PADDING.small, padding: PADDING.medium }}>
            <MinimizablePanel heading="Understanding the controls" color={COLORS.interact_color} startExpanded={true}>
                <p>
                    This page provides the controls for a single audio channel, which includes a preamp, high pass filter and a parametric Equalizer.
                </p>
                <h2>"In" buttons</h2>
                <p>
                    Many audio processing effects have an "In" button. This button is used to enable or disable the audio effect. When the button is "In" (green) the control is enabled, when the button is "Out" (grey) the control is disabled.
                </p>
                <h2>Preamp</h2>
                <p>
                    When a microphone is connected to the mixing desk, it's signal is often very quiet, and further processing (eg the paraetric equalizer) make it quieter still. In order to have
                    and audible signal coming out of the desk, when the signal arrives at the desk it is amplified by the "preamp". Next to the preamp knob there is a red LED labeled "Pk", this
                    LED lights up if the output of the preamp is peaking (too loud), which would cause the signal to distort. When picking a preamp level, you generally
                    want to make sure that the the signal doesn't peak, but is still loud enough to be used.
                </p>
                <h2>High Pass Filter (HPF)</h2>
                <p>
                    The high pass filter is used to remove low frequency noise from the signal (low frequencies are bass notes). Some common noises that can be picked up by a microphone that the High Pass Filter
                    is used to remove include wind blowing across the microphone, or "Pop's" from when a speaker says a word that starts with a 'P'.
                    Generally you want the high pass filter set as low as possible so it doens't remove any of the wanted audio, but high enough that it removes the noise. A high pass filter that is set
                    too high will make the sound thin and tinny.
                </p>
                <p>
                    The high pass filter can be seen on the screen as a purple line.
                </p>
                ...
                <div style={{ display: 'flex', alignItems: 'start', gap: PADDING.medium }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: PADDING.medium }}>
                        <Panel heading="Preamp">
                            <PreampPanel preamp={mixerSettings.preamp} preampLevel={preampLevel} onChangePreamp={(mod) => setMixerSettings((prev) => ({
                                ...prev,
                        ...
                        <Panel heading="HPF">
                            <HighPassFilterPanel highPassFilter={mixerSettings.highPassFilter} onChangeHighPassFilter={(mod) => setMixerSettings((prev) => ({
                                ...prev,
