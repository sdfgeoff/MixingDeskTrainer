...
const AudioPanelMulti: React.FC<AudioPanelProps> = ({
  audioTracks,
  setAudioSource,
  setAudioContext,
  setMixerModel,
  ...
  const dataUrl =
    audioTrack?.source === "default" ? audioTrack?.src : undefined;
  ...
  const audioUrl = mixerModel?.source.audioUrl.startsWith("blob:")
    ? mixerModel?.source.audioUrl
    : `${basePath}/${mixerModel?.source.audioUrl}`;
  ...
  return (
    <div style={{ display: "flex", gap: PADDING.small }}>
      ...
      {modelResponse.state === "error" && (
        <div>Error: {modelResponse.errorMessage}</div>
      )}
