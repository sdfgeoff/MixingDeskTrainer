...
import { ChannelNodes, createChannelNodes, syncChannelProcessingToMixerModel } from "./InputChannelProcessing";
...
export const createMixingDesk = (audioContext: AudioContext, numChannels: number, parametricEqSize: number, numBusses: number, numBands: number): MixingDeskNodes => {
    const channels = Array.from({ length: numChannels }, () => createChannelNodes(audioContext, parametricEqSize))
    const busses = Array.from({ length: numBusses }, () => createMixBus(audioContext, numBands))
...
export const useSyncMixingDeskToMixerModel = (mixingDeskNodes: MixingDeskNodes | undefined, mixerModel: MixerModel): void => {
    useEffect(() => {
    ...
    const busBandChannelMapping: { busIndex: number, bandIndex: number, channelIndex: number }[] = useDebouncedObject(mixerModel.busses.flatMap((bus, busIndex) => {
