...
const COLORS = ["#00FF00", "#00EEEE", "#3333FF", "#FF0000"];
...
const DB_GRID_LINES = [-12, -6, 0, 6, 12];
...
const deriveHighpassFilterConstants = (
...
const evaluateBiquad = (filterConstants: FilterConstants, freq: number) => {
  ...
  const phi = Math.pow(
    Math.sin((2.0 * Math.PI * freq) / (2.0 * FAKE_SAMPLE_RATE)),
    2.0,
  );
  const r =
    (Math.pow(b0 + b1 + b2, 2.0) -
      4.0 * (b0 * b1 + 4.0 * b0 * b2 + b1 * b2) * phi +
      16.0 * b0 * b2 * phi * phi) /
    (Math.pow(1.0 + a1 + a2, 2.0) -
...
const EQView: React.FC<EQViewProps> = ({ eqSettings, highPassFilter }) => {
  ...
  const totalResponse = React.useMemo(() => {
    return frequencyPoints.map((_f, i) =>
      parametersWithSamples.reduce((acc, p) => acc + p.samples[i], 0),
    );
  }, [parametersWithSamples, frequencyPoints]);

  return (
    <svg
      viewBox={`0 0 ${PIXELS_X} ${PIXELS_Y}`}
      width={PIXELS_X}
      height={PIXELS_Y}
    ...
    >
      ...
      {parametersWithSamples.map((p, i) => {
        ...
        return p.samples.map((s, j) => {
          ...
          if (s > 0) {
            return (
              <rect
                key={`bar-positive-${i}-${j}`}
                x={freqToX(frequencyPoints[j]) - i * 2}
                y={gainY}
                width={2}
                height={PIXELS_Y / 2 - gainY}
                fill={eqSettings.enabled ? p.color : GRIDLINE_COLOR}
              />
            );
          } else {
            return (
              <rect
                key={`bar-negative-${i}-${j}`}
                x={freqToX(frequencyPoints[j]) - i * 2}
                y={PIXELS_Y / 2}
                width={2}
                height={Math.abs(gainY - PIXELS_Y / 2)}
                fill={eqSettings.enabled ? p.color : GRIDLINE_COLOR}
              />
            );
          }
      ...
      {/* HighPass Filter */}
      <polyline
        points={highPassFilterPoints
          .map((v, i) => `${freqToX(frequencyPoints[i])},${gainToY(v)}`)
          .join(" ")}
        stroke={highPassFilter.enabled ? "#ff00ff" : GRIDLINE_COLOR}
        strokeWidth={1}
        fill="none"
      />
      ...
      {/* Total response */}
      <polyline
        points={totalResponse
          .map((v, i) => `${freqToX(frequencyPoints[i])},${gainToY(v)}`)
          .join(" ")}
        stroke={eqSettings.enabled ? "yellow" : GRIDLINE_COLOR}
        strokeWidth={1}
        fill="none"
      />
      ...
      {parametersWithSamples.map((p, i) => {
        ...
        return (
          <rect
            key={`sample-rect-${i}`}
            x={freqToX(p.frequency) - 2}
            y={gainToY(p.gainDb) - 2}
            width={5}
            height={5}
            stroke={eqSettings.enabled ? p.color : GRIDLINE_COLOR}
            strokeWidth={2}
            fillOpacity={0.0}
          />
        );
        ...
        {FREQ_GRID_LINES.map((freq, i) => {
          ...
          return (
            <text
              ...
              textAnchor="middle"
            >
              {freq.toString().replace(new RegExp("000$"), "k")}
            </text>
          );
        })}
      </>
