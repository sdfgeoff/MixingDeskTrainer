...
const FullDesk: React.FC = () => {
  ...
  const numBusses = mixerModel.busses.length;
  const numBands = mixerModel.busses[0].bands.length;

  const mixingDesk = useMemo(() => {
    if (audioContext) {
      return createMixingDesk(
        audioContext,
        numChannels,
        4,
        numBusses,
  ...
  }, [audioContext, numChannels, numBusses, numBands]);
  ...
  const outputNode =
    paflChannel === undefined
      ? mixingDesk?.busses[busId].outputGain
      : mixingDesk?.channels[paflChannel].parametricEq[
          mixingDesk?.channels[paflChannel].parametricEq.length - 1
        ];
  ...
  const updateChannelById = useCallback(
    (
      ...
      setMixerModel((oldModel) => {
        ...
        const linkedChannels = oldModel.channel_links
          .filter((link) => {
